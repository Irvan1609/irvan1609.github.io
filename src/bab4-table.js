...
function factorialHtml(report){
  ...
  return `<section class="bab4-result" data-bab4-table><div class="table-caption">Tabel hasil BAB IV. Rata-rata ${esc(measureName(report))} pada berbagai ${esc(aName)} dan ${esc(bName)}</div><div class="table-scroll"><table class="result-table bab4-table"><thead><tr><th>${esc(bName)} \\ ${esc(aName)}</th>${A.map(a=>`<th>${esc(a)}</th>`).join('')}<th>Rata-rata</th></tr></thead><tbody>${bRows}<tr><th scope="row">Rata-rata</th>${aRow}<td data-number="${report.grand}">${fmt(report.grand,2)}</td></tr></tbody></table></div><div class="analysis-note">${esc(note)}</div></section>`;
}
function oneFactorHtml(report){
  ...
  return `<section class="bab4-result" data-bab4-table><div class="table-caption">Tabel hasil BAB IV. Rata-rata ${esc(measureName(report))} pada berbagai ${esc(factor)}</div><div class="table-scroll"><table class="result-table bab4-table"><thead><tr><th>${esc(factor)}</th><th>Rata-rata</th><th>Keterangan perlakuan</th></tr></thead><tbody>${rows}</tbody></table></div><div class="analysis-note">${esc(note)}</div></section>`;
}
