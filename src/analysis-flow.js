...
export function installAnalysisFlow() {
  ...
  const panel=document.createElement('div');panel.id='analysisMenu';panel.className='nav-command-panel';panel.hidden=true;panel.setAttribute('role','group');panel.setAttribute('aria-label','Analisis');
  [...choice.querySelectorAll('[data-design]')].forEach(button=>panel.append(button));$('.nav').after(panel);
  $('#openAnalysis').textContent='Analisis';
  panel.insertAdjacentHTML('beforeend','<button data-design-ext="nested"><b>Rancangan Tersarang</b><span>B tersarang dalam A</span></button><button data-design-ext="repeated"><b>Repeated Measures</b><span>Pengamatan berkala / waktu</span></button><button data-nonparametric><b>Nonparametrik</b><span>Kruskal–Wallis / Friedman</span></button><button data-association="correlation"><b>Korelasi</b><span>Pearson / Spearman</span></button><button data-association="path"><b>Sidik lintas</b><span>Satu Y, beberapa X</span></button><button data-advanced="descriptive"><b>Statistik Deskriptif</b><span>Mean, SD, SE, CV, skewness</span></button><button data-nextgen="regression"><b>Regresi</b><span>Kurva linear, kuadratik, kubik</span></button><button data-power><b>Power & Jumlah Ulangan</b><span>Perencanaan ANOVA satu faktor</span></button><button data-nextgen="pca"><b>PCA + Biplot</b><span>Scree plot, loading, PC1–PC2</span></button><button data-nextgen="combined"><b>ANOVA Gabungan</b><span>Seimbang atau GLM tidak seimbang</span></button><button data-mixed><b>Mixed Model REML</b><span>Kelompok(Lokasi) efek acak</span></button><button data-advanced="genetic"><b>Parameter Genetik</b><span>H², KKG, KKP, kemajuan genetik</span></button><button data-nextgen="stability"><b>AMMI / GGE Biplot</b><span>Stabilitas genotipe multilokasi</span></button><button data-stability-indices><b>Indeks Stabilitas</b><span>ASV, YSI, Wricke, Finlay–Wilkinson</span></button>');
  panel.querySelectorAll('[data-design-ext]').forEach(button=>button.addEventListener('click',()=>{closeAll();openDesignExtension(button.dataset.designExt);}));
  panel.querySelector('[data-nonparametric]').addEventListener('click',()=>{closeAll();openNonparametric();});
  panel.querySelector('[data-power]').addEventListener('click',()=>{closeAll();openPowerAnalysis();});
  panel.querySelector('[data-stability-indices]').addEventListener('click',()=>{closeAll();openStabilityIndices();});
