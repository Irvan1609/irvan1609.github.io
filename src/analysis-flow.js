...
const allItems=analysisGroups.flatMap(group=>group.items);
const keyOf=([type,value])=>type+':'+value;
const byKey=new Map(allItems.map(item=>[keyOf(item),item]));

function analysisButton(item){
  const [type,value,label,mark]=item,key=keyOf(item);
  return `<button type="button" class="analysis-grouped-item" data-analysis-open="${key}" title="${label}"><span>${mark}</span><b>${label}</b></button>`;
}
function panelMarkup(){
  return `<div class="analysis-menu-head"><div><b>Pilih analisis</b><small>Semua metode tersedia · dikelompokkan agar ringkas</small></div></div>
    <div class="analysis-grouped-grid">
      ${analysisGroups.map(group=>`<section class="analysis-grouped-section"><div class="analysis-grouped-title"><b>${group.title}</b><small>${group.items.length}</small></div><div class="analysis-grouped-items">${group.items.map(analysisButton).join('')}</div></section>`).join('')}
    </div>`;
}
...
export function installAnalysisFlow(){
  ...
  function openMenu(){
    ...
    requestAnimationFrame(()=>panel.querySelector('.analysis-grouped-item')?.focus());
  }
  ...
  document.addEventListener('agrotik-open-analysis',event=>{
    ...
    const item=byKey.get(key)||allItems.find(entry=>entry[1]===key);
