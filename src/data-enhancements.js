...
function fieldbookTool(){
  ...
  const createDataset=()=>{const out=current||make(),detail={name:`fieldbook-${$('#fieldDesign').value}`,headers:out.headers,rows:out.rows.map(r=>r.map(String))};document.dispatchEvent(new CustomEvent('dataset-import',{detail}));if(!detail.importResult?.ok)throw Error(detail.importResult?.error||'Dataset fieldbook belum dapat dibuat.');return detail;};
  $('#createFieldbook').onclick=()=>{try{createDataset();$('#dataToolModal').classList.remove('open');}catch(e){$('#fieldResult').innerHTML=`<div class="error-box">${esc(e.message)}</div>`;}};
  $('#createFieldbookMap').onclick=async()=>{try{createDataset();$('#dataToolModal').classList.remove('open');const {openFieldLayout}=await import('./field-layout.js');openFieldLayout();}catch(e){$('#fieldResult').innerHTML=`<div class="error-box">${esc(e.message)}</div>`;}};
...
export function installDataEnhancements(){
  ...
  const openFieldHash=async()=>{
    const qr=location.hash.match(/^#p=([^&]+)/),returning=location.hash==='#field-return';if(!qr&&!returning)return;
    try{
      const {openFieldLayout}=await import('./field-layout.js');
      openFieldLayout(qr?{uidPrefix:decodeURIComponent(qr[1])}:{});
      if(returning)history.replaceState(null,'',location.pathname+location.search);
    }catch(error){console.error(error);}
  };
  window.addEventListener('hashchange',openFieldHash);if(location.hash.startsWith('#p=')||location.hash==='#field-return')setTimeout(openFieldHash,0);
  updateUndoButtons();
}
