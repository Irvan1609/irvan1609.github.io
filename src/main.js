...
function showError(message, error = null) {
  ...
  const text = `⚠ ERROR: ${message}${error?.message ? ` — ${error.message}` : ''}`
...
function saveData() {
  try {
    ...
    updateStorageStatus()
  } catch (err) { showError('Gagal menyimpan data ke penyimpanan browser.', err) }
}
...
function loadData() {
  ...
  } catch (err) { showError('Data tersimpan tidak dapat dipulihkan.', err) }
  return false
}
    ...
    <div class="app-shell d-flex flex-column vh-100">
      <header class="border-bottom bg-white">
        <div class="container-fluid py-2 d-flex align-items-center gap-2">
          <button id="menuBtn" class="btn btn-outline-secondary d-lg-none" aria-label="Project Explorer">☰</button>
      ...
      <main class="d-flex flex-grow-1 min-h-0 overflow-hidden">
        <aside class="project-panel border-end bg-white" id="projectPanel">
          <div class="offcanvas-header border-bottom"><h6 class="offcanvas-title mb-0">Project Explorer</h6><button id="closePanel" class="btn-close d-lg-none" aria-label="Tutup"></button></div>
          <div class="p-2">
      ...
      <nav class="mobile-nav d-lg-none border-top bg-white">
        ...
        <button id="moreBtn" class="mobile-nav-item">☰<span>More</span></button>
      </nav>
    </div>
    ...
    <div class="modal fade" id="pasteModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered modal-lg modal-fullscreen-sm-down">
        <div class="modal-content">
          <div class="modal-header"><h5 class="modal-title">Paste Data dari Excel</h5><button id="closeModal" class="btn-close" aria-label="Tutup"></button></div>
          ...
          <div class="modal-footer"><button id="cancelPaste" class="btn btn-outline-secondary">Batal</button><button id="applyPaste" class="btn btn-primary">Masukkan Data</button></div>
        </div>
      </div>
    </div>`
  bind()
...
function openPasteModal() {
  const modal = document.querySelector('#pasteModal')
  if (!modal) return showError('Komponen Paste Data tidak ditemukan.')
...
function toggleProjectPanel(force) {
  const panel = document.querySelector('#projectPanel')
  if (!panel) return showError('Project Explorer tidak ditemukan.')
...
function bind() {
  try {
    document.querySelector('#pasteBtn').onclick = () => { try { clearError(); document.querySelector('#pasteArea').value=''; document.querySelector('#preview').innerHTML=''; openPasteModal() } catch(err){ showError('Tombol Paste from Excel gagal dibuka.',err) } }
    document.querySelector('#closeModal').onclick = closePasteModal
    document.querySelector('#cancelPaste').onclick = closePasteModal
    ...
    document.querySelector('#addRow').onclick = () => { try { clearError(); if(!state.headers.length)state.headers=['Variable1']; state.rows.push(state.headers.map(()=>'')); saveData(); renderGrid(); document.querySelector('#status').textContent='✓ Baris baru ditambahkan.' } catch(err){ showError('Gagal menambahkan baris.',err) } }
    document.querySelector('#addCol').onclick = () => { try { clearError(); state.headers.push(`Variable${state.headers.length+1}`); state.rows.forEach(r=>r.push('')); saveData(); renderGrid(); document.querySelector('#status').textContent='✓ Kolom baru ditambahkan.' } catch(err){ showError('Gagal menambahkan kolom.',err) } }
    document.querySelector('#clearData').onclick = () => { try { clearError(); if(!state.headers.length)return showError('Tidak ada dataset yang dapat dihapus.'); if(confirm('Hapus dataset sementara dari browser?'))clearSavedData() } catch(err){ showError('Tombol Hapus Data gagal dijalankan.',err) } }

    document.querySelector('#menuBtn')?.addEventListener('click',()=>toggleProjectPanel())
    document.querySelector('#moreBtn')?.addEventListener('click',()=>toggleProjectPanel())
    document.querySelector('#closePanel')?.addEventListener('click',()=>toggleProjectPanel(false))
    document.querySelectorAll('[data-action]').forEach(button=>button.addEventListener('click',()=>{
      const name=button.dataset.name||'Menu'
      if(name!=='Data') showError(`Fitur ${name} belum tersedia. Tombol berfungsi dan menampilkan pesan ini.`)
      else { clearError(); document.querySelector('#status').textContent='✓ Menu Data aktif.' }
      if(window.innerWidth<992 && button.classList.contains('tree-item')) toggleProjectPanel(false)
    }))
