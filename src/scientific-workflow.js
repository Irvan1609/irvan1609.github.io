...
function installResultControls(container,reports,datasetName){
  ...
  const viewButtons=[...container.querySelectorAll('[data-simple-result-view]')];
...
function showResults(reports,container,datasetName=reports[0]?.datasetName||'hasil-analisis',meta={}){
  ...
  container.innerHTML=`<div class="analysis-stale-banner" data-stale-banner ${stale?'':'hidden'}><span>Data berubah · hasil perlu dihitung ulang</span><button type="button" data-rerun-stale>Hitung ulang</button></div><div class="analysis-result-toolbar"><div class="simple-result-tabs" role="tablist" aria-label="Tampilan hasil"><button type="button" data-simple-result-view="summary" aria-pressed="true">Ringkas</button><button type="button" data-simple-result-view="anova" aria-pressed="false">ANOVA</button><button type="button" data-simple-result-view="diagnostic" aria-pressed="false">Diagnostik</button><button type="button" data-simple-result-view="full" aria-pressed="false">Lengkap</button></div><select data-result-focus aria-label="Fokus parameter"><option value="">Semua parameter</option>${focusOptions}</select>${resultVersion?`<span class="analysis-version-badge">V${resultVersion}</span>`:''}<details class="result-tools-menu"><summary>Alat</summary><div class="result-tools-menu-body"><div class="result-significance-filter" role="group" aria-label="Filter signifikansi"><button type="button" data-result-filter="all" aria-pressed="true">Semua</button><button type="button" data-result-filter="ss" aria-pressed="false">**</button><button type="button" data-result-filter="s" aria-pressed="false">*</button><button type="button" data-result-filter="tn" aria-pressed="false">tn</button></div><button type="button" data-compare-mode aria-pressed="false">Bandingkan</button><button type="button" data-thesis-table-mode aria-pressed="false">Skripsi</button><button type="button" data-publication-mode aria-pressed="false">Publikasi</button><button type="button" data-presentation-mode aria-pressed="false">Presentasi</button></div></details><div class="mobile-result-nav"><button type="button" data-result-prev aria-label="Parameter sebelumnya">‹</button><span data-result-page></span><button type="button" data-result-next aria-label="Parameter berikutnya">›</button></div></div><details class="result-export-tools"><summary>Ekspor / pemeriksaan</summary><div class="result-actions master-result-actions"><button data-result-action="export-bab4">BAB IV</button><button data-result-action="export-all">Ekspor .xlsx</button><button type="button" data-print-results>Cetak / PDF</button><button type="button" data-thesis-check>Periksa</button><button data-result-action="export-all-formula">Ekspor formula</button><span role="status" class="export-status"></span></div></details><div data-thesis-audit-host></div>${renderAnalysisSummary(ordered)}${ordered.map(renderReport).join('')}`;
...
function syncSimpleParameter(forceSingle=false){
  const select=$('#scienceSimpleParameter');if(!select)return;
  const inputs=[...document.querySelectorAll('#scienceParameters input')].filter(input=>!input.disabled&&isNumericColumn(Number(input.value)));
  const previous=select.value;
  select.innerHTML=inputs.map(input=>`<option value="${input.value}">${esc(data.headers[Number(input.value)])}</option>`).join('');
  let selected=inputs.find(input=>input.value===previous)||inputs.find(input=>input.checked)||inputs[0];
  if(selected)select.value=selected.value;
  if(forceSingle&&selected){
    inputs.forEach(input=>{input.checked=input===selected;const transform=document.querySelector(`[data-transform-param="${input.value}"]`);if(transform)transform.disabled=input!==selected;});
  }
