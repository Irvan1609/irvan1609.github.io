...
(() => {
  ...
  function open() {
    ...
    $('#ralReplicate').innerHTML='<option value="">Tidak ada kolom ulangan</option>'+dataset().headers.map((h,i)=>`<option value="${i}">${esc(h)}</option>`).join('');
    $('#ralTreatment').insertAdjacentHTML('afterbegin','<option value="">Pilih perlakuan</option>');
    $('#ralTreatment').value='';
    $('#ralResult').innerHTML = '';
  ...
  function analyze() {
    ...
    const alpha = Number($('#ralAlpha')?.value), posthoc=$('#ralPosthoc').value;
    const ri=$('#ralReplicate').value===''?null:Number($('#ralReplicate').value);
    if (!selected.length) return err('Pilih minimal satu peubah respons.');
    if ($('#ralTreatment').value==='' || !Number.isInteger(ti)) return err('Pilih peubah perlakuan.');
    if(ri!==null&&(ri===ti||selected.includes(ri)))return err('Perlakuan, ulangan, dan parameter harus berbeda.');
    ...
    for (const yi of selected) {
      ...
      const obs = [],seen=new Set();
      for (const [rowIndex,r] of d.rows.entries()) {
        if(!r.some(v=>String(v??'').trim()))continue;
        const t = String(r[ti]??'').trim();
        if(!t)return err(`Perlakuan kosong pada baris ${rowIndex+1}.`);
        const rep=ri===null?null:String(r[ri]??'').trim();
        if(ri!==null){if(!rep)return err(`Ulangan kosong pada baris ${rowIndex+1}.`);const key=JSON.stringify([t,rep]);if(seen.has(key))return err(`Pengamatan ganda: ${t}, ulangan ${rep}.`);seen.add(key);}
        ...
        if (t !== '' && Number.isFinite(y)) obs.push({t,y,rep});
      }
      ...
      const replicateNames=ri===null?Array.from({length:Math.max(...groups.map(g=>g.length))},(_,i)=>String(i+1)):[...new Set(obs.map(o=>o.rep))];
      const maxRep=replicateNames.length;
      const displayGroups=ri===null?groups:treatments.map(t=>replicateNames.map(rep=>obs.find(o=>o.t===t&&o.rep===rep)?.y));
    ...
    if (dfE < 1 || !Number.isFinite(msE) || msE <= 0) {
        html += `<section class="ral-result-block analysis-result" data-export-scope><h3>${esc(responseName)}</h3>${resultActions(`RAL-${safeFilename(responseName)}`)}<div class="analysis-note">Analisis memerlukan db galat minimal 1 dan KT galat positif.</div></section>`;
        continue;
      }
      ...
      const repHeaders = replicateNames.map(rep=>`<th>Ulangan ${esc(rep)}</th>`).join('');
      const observationRows = treatments.map((t,i)=>`<tr><td>${esc(t)}</td>${Array.from({length:maxRep},(_,r)=>`<td>${displayGroups[i][r] === undefined ? '' : fmt(displayGroups[i][r],2)}</td>`).join('')}<td>${fmt(totals[i],2)}</td><td>${fmt(meanValues[i],2)}</td></tr>`).join('');
      const repTotals = Array.from({length:maxRep},(_,r)=>displayGroups.reduce((sum,g)=>sum+(g[r] ?? 0),0));
      ...
      if (significantSelected && posthoc!=='none') {
        const method=posthoc==='bnt'?'BNT':'BNJ';
        let q = NaN;
        try { q = posthoc==='bnt'?jStat.studentt.inv(1-alpha/2,dfE):jStat.tukey.inv(1-alpha,a,dfE); } catch {}
        if (!Number.isFinite(q) || q <= 0) return err(`Nilai kritis tidak dapat dihitung untuk ${responseName}.`);
        const significant = Array.from({length:a},()=>Array(a).fill(false));
        for (let i=0;i<a;i++) for (let j=i+1;j<a;j++) {
          const sePair = Math.sqrt(msE/(posthoc==='bnt'?1:2)*(1/groups[i].length+1/groups[j].length));
        ...
        const hsd = equalN ? q*se*(posthoc==='bnt'?Math.SQRT2:1) : NaN;
        ...
        section += `<div class="analysis-note"><b>${method} ${alpha*100}%</b>: ${posthoc==='bnt'?'t':'q'} tabel = ${fmt(q)} (k = ${a}, db galat = ${dfE})${equalN?`, SE = ${fmt(se)}, nilai ${method} = ${fmt(hsd)}`:(posthoc==='bnt'?', batas BNT dihitung per pasangan sesuai jumlah ulangan':', menggunakan Tukey–Kramer karena jumlah ulangan tidak sama')}. Huruf superscript yang sama menunjukkan tidak berbeda nyata.</div><div class="table-caption">Tabel ${meansNo}. Rata-Rata ${esc(responseName)} pada Berbagai Perlakuan</div><table class="result-table posthoc-table"><thead><tr><th>Perlakuan</th><th>${esc(responseName)}</th></tr></thead><tbody>${meanRows}</tbody></table><div class="analysis-note">Rata-rata tertinggi terdapat pada ${esc(treatments[high])} (${fmt(meanValues[high],2)}), sedangkan terendah pada ${esc(treatments[low])} (${fmt(meanValues[low],2)}). Kedua perlakuan tersebut ${related?'masih memiliki huruf yang sama sehingga tidak berbeda nyata':'tidak memiliki huruf yang sama sehingga berbeda nyata'} pada ${method} ${alpha*100}%. Urutan hasil mengikuti urutan perlakuan pada dataset.</div>`;
      } else if(posthoc==='none') {
        section+='<div class="analysis-note">Uji lanjut tidak dilakukan sesuai pilihan Anda.</div>';
        section+=`<table class="result-table posthoc-table"><thead><tr><th>Perlakuan</th><th>${esc(responseName)}</th></tr></thead><tbody>${treatments.map((t,i)=>`<tr><td>${esc(t)}</td><td>${fmt(meanValues[i],2)}</td></tr>`).join('')}</tbody></table>`;
      } else {
        const high = meanValues.indexOf(Math.max(...meanValues));
        const low = meanValues.indexOf(Math.min(...meanValues));
  ...
  function bind() {
    installResultExport();
    document.addEventListener('open-analysis-ral',open);
