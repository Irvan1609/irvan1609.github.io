...
(() => {
  ...
  function ensureBnjOptions(){
    ...
    box.innerHTML='<div><label for="ralAlpha" style="display:block;font-weight:700;margin-bottom:5px">Taraf nyata BNJ (α)</label><select id="ralAlpha"><option value="0.05">0.05 (5%)</option><option value="0.01">0.01 (1%)</option></select></div><div style="font-size:13px;color:#526171">q tabel dihitung otomatis dari taraf nyata, jumlah perlakuan, dan db galat masing-masing respons. Hasil mengikuti urutan perlakuan pada data.</div>';
  ...
  function analyze(){
    ...
    for(const yi of selected){
      ...
      if(dfE<2||!Number.isFinite(msE)||msE<=0){html+=`<div class="analysis-note">${esc(d.headers[yi])}: BNJ otomatis memerlukan db galat minimal 2 dan KT galat positif.</div>`;continue;}
      let q;try{q=jStat.tukey.inv(1-alpha,a,dfE);}catch{q=NaN;}
      if(!Number.isFinite(q)||q<=0){html+=`<div class="analysis-note">q tabel untuk ${esc(d.headers[yi])} tidak dapat dihitung.</div>`;continue;}
      let posthoc=`<div class="analysis-note">q = ${fmt(q)} (perlakuan = ${a}, db galat = ${dfE}, α = ${fmt(alpha,2)}). Uji BNJ tidak dijalankan karena pengaruh perlakuan tidak nyata.</div>`;
      ...
      if(p<alpha){
        ...
        posthoc=`<div class="analysis-note"><b>BNJ ${alpha*100}%</b>: q = ${fmt(q)} (perlakuan = ${a}, db galat = ${dfE})${equalN?`, SD = ${fmt(sd)}, NP = ${fmt(np)}`:', menggunakan penyesuaian Tukey–Kramer karena ukuran kelompok tidak sama'}.</div>`;
      }
      const order=meanValues.map((m,i)=>({i,m}));
      ...
      html+=`<div class="ral-result-block"><h4>${esc(d.headers[yi])}</h4><table class="result-table"><thead><tr><th>Sumber</th><th>db</th><th>JK</th><th>KT</th><th>F hitung</th><th>p-value</th></tr></thead><tbody><tr><td>Perlakuan</td><td>${dfT}</td><td>${fmt(ssT)}</td><td>${fmt(msT)}</td><td>${fmt(f)}</td><td>${p<.001?'&lt;'+fmt(0.001):fmt(p)}</td></tr><tr><td>Galat</td><td>${dfE}</td><td>${fmt(ssE)}</td><td>${fmt(msE)}</td><td>—</td><td>—</td></tr><tr><td>Total</td><td>${N-1}</td><td>${fmt(ssTot)}</td><td>—</td><td>—</td><td>—</td></tr></tbody></table>${posthoc}<table class="result-table"><thead><tr><th>Perlakuan</th><th>n</th><th>Rataan</th><th>Notasi BNJ</th></tr></thead><tbody>${means}</tbody></table><div class="analysis-note">Rataan yang memiliki sekurang-kurangnya satu huruf sama tidak berbeda nyata. Hasil mengikuti urutan kemunculan perlakuan pada data. Kesimpulan α = ${fmt(alpha,2)}: perlakuan <b>${p<alpha?'berpengaruh nyata':'tidak berpengaruh nyata'}</b> terhadap ${esc(d.headers[yi])}.</div></div>`;
    }
