...
const st=stabilityAnalysis(stabilityRows);assert.equal(st.environments.length,3);assert.equal(st.genotypes.length,4);assert.equal(st.n,24);
...
assert.ok(st.ammi.components[0].percent>0);assert.ok(st.gge.components[0].percent>0);assert.equal(st.ammi.genotypes.length,4);assert.equal(st.gge.environments.length,3);
assert.throws(()=>stabilityAnalysis(stabilityRows.filter(([e,g])=>!(e==='E3'&&g==='G4'))),/tidak memiliki pengamatan/);
console.log('Flexible combined ANOVA switches balanced data to classical ANOVA and unequal replication to Type III GLM; AMMI/GGE singular-value decomposition preserves interaction and G+GE sums of squares.');
