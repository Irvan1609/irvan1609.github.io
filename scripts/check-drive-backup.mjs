...
globalThis.localStorage={getItem:key=>key==='statistical_web_field_layout_v1'?JSON.stringify({'Test.csv':{version:3,backupMeta:true,plotUids:['u1'],statuses:{u1:'disease'},notes:{u1:'cek daun'}}}):null};
const bytesWithField=await rawWorkbook({...data,fileName:'Test.csv'}),bookWithField=new ExcelJS.Workbook();await bookWithField.xlsx.load(bytesWithField);
assert.equal(bookWithField.worksheets.length,2);
assert.equal(bookWithField.worksheets[1].name,'Metadata denah');
assert.match(String(bookWithField.worksheets[1].getCell('B5').value||''),/plotUids/);
