...
for(const marker of ['data-analysis-unified-view','data-analysis-more','data-analysis-search','openMore()',"if(button)await openButton(button)"])if(!flow.includes(marker))fail('single compact analysis selection missing '+marker);
for(const obsolete of ['confirmAnalysis','analysisSelectedLabel','data-analysis-mode-toggle','Mode Lengkap','Mode Sederhana'])if(flow.includes(obsolete))fail('obsolete analysis selection control remains '+obsolete);
if(!statStyle.includes('/* STAT SINGLE COMPACT UI 2026-09-26 */')||!statStyle.includes('.analysis-more-toolbar')||!statStyle.includes('.analysis-more-catalog'))fail('single compact analysis styles missing');
...
for(const marker of ["FAVORITES='statistical_web_analysis_favorites_v1'","RECENT='statistical_web_analysis_recent_v1'","USAGE='statistical_web_analysis_usage_v1'","data-auto-detect","data-quick-run","data-favorite-key","data-analysis-search"])if(!flow.includes(marker))fail('analysis favorite/recent/search workflow missing '+marker);
...
if(scientific.includes('scienceModeToggle')||scientific.includes('Mode Lengkap')||scientific.includes('Mode Sederhana'))fail('scientific analysis must use one interface without mode switching');
