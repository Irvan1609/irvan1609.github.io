...
for(const marker of ['id="fieldGrid"','id="vaultList"','id="crossSeeds"','id="eventModal"','id="recapModal"','id="metaModal"','id="socialModal"','id="toolDock"','id="quickSocial"','id="labHub"','id="playHint"','id="selectedSeedQuick"','id="gameHelp"','id="harvestBasket"','data-field-tool="harvest"','id="quickExperiment"','id="experimentTool"','data-field-tool="treatment"','Peringkat','Koleksi Benih','Teknologi','Periksa','/account.js','/game/app.js','/game/social.js','/game/style.css','/pwa-register.js'])if(!html.includes(marker))fail('game shell missing '+marker);
for(const marker of ['agrotik_field_zero_v1','finishSeason','crossSeeds','LOCATIONS','TECH','EXPEDITIONS','CHALLENGES','BOSSES','RIVALS','dailyDefinition','doPrestige','startExpedition','startGenomePuzzle','openWorldMap','openQuickMore','openMusicPicker','setFieldTool','useFieldTool','fieldzero/seed','fieldzero/tool','applyRemoteBurn','applyWaterAid','harvestReason','firstHarvestBoost','stressLog','data-rename-seed','harvestCombo','fieldzero/harvest-index','toolSymbol','isMusicPlaying','fieldzero-profile','FieldZeroGame','STAT_IMPORT_KEY','openExperiment','createExperiment','randomizedExperimentUnits','experimentDataset','sendExperimentToStat','recordExperimentObservation','applyExperimentTreatment','experimentSeedForPlot','createBreedingCup','breedingCup.active','trial24','attentionPlots'])if(!app.includes(marker))fail('gameplay/meta progression loop missing '+marker);
for(const marker of ['.competition-head','.competition-diagnostics','.decision-review','.soil-high','.field-grid','.plot.ready','.plot.sick','.event-backdrop','.seed-list','.game-quick-nav','.field-tool-dock','.drag-target','.plot.burned','.social-card','.social-tabs','.social-player','.aid-button','.music-picker','.seed-card-actions','.basket-ready','.dragging-harvest','.experiment-badge','.experiment-form','.experiment-table','.experiment-actions','.lab-hub','.play-hint','.field-context','.help-steps','@media(max-width:620px)'])if(!css.includes(marker))fail('game styling missing '+marker);
...
if(/fetch\(|XMLHttpRequest|WebSocket/.test(app+competition))fail('Core Field Zero simulation must remain local/offline-first');
for(const marker of ['G + E + M','G×E','G×M','24*135000','TENDERS','STRATEGIES','DIAGNOSTICS','DATA SIMULASI','Uji akhir rahasia','Pilih tepat 2 galur','competitionFinal'])if(!competition.includes(marker))fail('Breeding Cup missing '+marker);
...
for(const marker of ['agrotik_stat_import_queue_v1','consumeExternalDatasetImport','source!==\'field-zero\'','dataset-import'])if(!statMain.includes(marker))fail('Field Zero to /stat bridge missing '+marker);
console.log('Field Zero check OK: RAL/RAK plus 24-plot blind-selection Breeding Cup and local /stat simulation transfer are wired.');
