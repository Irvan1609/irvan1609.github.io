...
import {generationStage,phenotypeState,dailyBreedingPlan} from '../public/game/academy-plus.js';
...
const academyPlus=fs.readFileSync('public/game/academy-plus.js','utf8');
...
if(/fetch\(|XMLHttpRequest|WebSocket/.test(app+competition+academy+academyPlus))fail('Core Field Zero simulation must remain local/offline-first');
...
for(const marker of ['G + E + M','G×E','G×M','24*135000','TENDERS','STRATEGIES','DIAGNOSTICS','DATA SIMULASI','Uji akhir rahasia','Pilih tepat 2 galur','finalTest','INVESTIGASI KEGAGALAN','name="design"','dailyKey','daily-breeding:'])if(!competition.includes(marker))fail('Breeding Cup missing '+marker);
...
for(const marker of ['speciesProfile','recommendedParameters','makeSubsamples','sampleMeasurements','aggregateSamples','plotCarryover','evidenceLabel','founderGenome','crossGenome','selfGenome','geneticEffects','analyzeExperiment','auditDesign','makeProgeny','genomeStats','heritability','repeated-measure','LINKAGE_GROUPS','linkedGamete','recombination','inbreeding','heterosis'])if(!academy.includes(marker))fail('academy model missing '+marker);
for(const marker of ['generationStage','phenotypeState','contractBrief','rivalProgramStatus','dailyBreedingPlan'])if(!academyPlus.includes(marker))fail('advanced academy helper missing '+marker);
...
const f2Stage=generationStage({generation:2,generationLabel:'F2',parents:['A','B'],homozygosity:.5});
if(f2Stage.phase!=='Segregasi tinggi'||f2Stage.scope!=='Seleksi individu')fail('F2 breeding stage invalid');
const phenotype=phenotypeState({water:20,n:25,disease:35,health:92,growth:60,seed:{vigor:1.05},selectionMarked:true});
for(const marker of ['phenotype-dry','phenotype-low-n','phenotype-disease','selection-marked'])if(!phenotype.classes.includes(marker))fail('phenotype cue missing '+marker);
const dailyPlan=dailyBreedingPlan('2026-09-26',12345);if(dailyPlan.strategy!=='balanced'||dailyPlan.budget!==12000000)fail('daily breeding plan invalid');

console.log('Field Zero check OK: professor academy, direct field selection, phenotype cues, staged breeding, linkage/recombination, inline ANOVA, academic reputation, thematic contracts, rival progression and deterministic Daily Breeding are wired.');
