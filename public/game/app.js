...
function updateCrossPreview(){
  ...
  const possible=unique([...a.traits,...b.traits]),nextStage=generationStage({generation:Math.max(a.generation,b.generation)+1,parents:[a.id,b.id]});$('#crossPreview').innerHTML=`<b>${esc(nextStage.label)} · ${esc(nextStage.phase)}</b><small>Heterosis dapat muncul pada lokus heterozigot; lokus YLD–MAT dan WUE–DIS terpaut sehingga rekombinan unggul tidak selalu muncul.</small><span class="trait-row">${possible.map(id=>`<span class="trait ${traitMeta(id).rarity}">${esc(traitMeta(id).name)}</span>`).join('')}</span>`;
}
function crossSeeds(){
