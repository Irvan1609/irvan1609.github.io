...
function ensure(){
  ...
  master=ctx.createGain();master.gain.value=.22;master.connect(ctx.destination);return ctx;
}
...
function schedule(){
  ...
  while(nextTime<c.currentTime+.25){
    ...
    voice(lead,nextTime,beat*.8,track.wave,.11);
    if(step%2===0)voice(bass,nextTime,beat*1.7,'sine',.06);
    if(step%4===0){
      const fifth=hz(bass)*1.5,c2=c.createOscillator(),g=c.createGain();
      c2.type='sine';c2.frequency.value=fifth;g.gain.setValueAtTime(.0001,nextTime);g.gain.exponentialRampToValueAtTime(.035,nextTime+.03);g.gain.exponentialRampToValueAtTime(.0001,nextTime+beat*1.6);
      c2.connect(g);g.connect(master);c2.start(nextTime);c2.stop(nextTime+beat*1.7);
    }
    nextTime+=beat;step++;
  }
}
export async function startMusic(track=current){
  current=TRACKS[track]?track:'morning';const c=ensure();
  try{if(c.state==='suspended')await c.resume();}catch{}
  playing=true;step=0;nextTime=c.currentTime+.05;
...
export function isMusicPlaying(){return playing&&!!ctx&&ctx.state==='running';}
