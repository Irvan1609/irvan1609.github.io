...
export function createBreedingCup(api){
  ...
  function start(values){
    ...
    st.experiment={id:api.uid('exp'),name:'Breeding Cup · '+tender.name,design:'rak',kind:'competition',treatments,reps:strategy.reps,parameters:[...PARAMETERS],units:api.randomize('rak',treatments,strategy.reps),createdAt:new Date().toISOString()};
    st.competition={id:api.uid('cup'),seed,strategy:values.strategy,tender:values.tender,management:values.management,reps:strategy.reps,initialBudget,budget:initialBudget-cost,spent:cost,stage:'design',trialEnv:ENVS[Math.floor(unit(seed,'trial-env')*ENVS.length)].id,hiddenLines:Array.from({length:strategy.lines},(_,i)=>line(seed,i)),diagnostics:[],selected:[],decisions:[{type:'design',text:strategy.name+' · '+management.name}],final:null};
  ...
  function diagnosticText(type){
    ...
    if(type==='soil')return 'Gradien kesuburan meningkat ke sisi kanan lahan; kelembapan cenderung meningkat ke baris bawah. Pengelompokan membantu menahan bias posisi.';
    if(type==='plant'){
  ...
  function open(){
    ...
    if(!comp){
      ...
      api.openModal('BREEDING CUP','🏆 Seleksi varietas dengan anggaran terbatas','<form id="competitionForm" class="experiment-form competition-form"><label class="experiment-wide">Strategi 24 petak<select name="strategy">'+strategyOptions+'</select></label><label>Tender<select name="tender">'+tenderOptions+'</select></label><label>Manajemen<select name="management">'+managementOptions+'</select></label><div class="competition-budget experiment-wide"><span>Anggaran awal</span><b>'+money(12000000)+'</b><small>Biaya rancangan, input, dan diagnostik mengurangi anggaran. Potensi genetik galur disembunyikan.</small></div><button class="primary experiment-wide" type="submit">🎲 Mulai kompetisi 24 petak</button></form><p class="meta-note">Model menghasilkan data dari G + E + M + G×E + G×M + gradien kesuburan/kelembapan + galat. Tidak ada aturan yang memaksa p-value menjadi nyata.</p>');
      document.querySelector('#competitionForm').onsubmit=event=>{event.preventDefault();const fd=new FormData(event.currentTarget);start({strategy:String(fd.get('strategy')),tender:String(fd.get('tender')),management:String(fd.get('management'))});};
      return;
    }
    ...
    let body='<div class="competition-head"><span>🏆 '+esc(tender.name)+'</span><span>📐 '+esc(plan.name)+'</span><span>💰 '+money(comp.budget)+'</span></div><div class="competition-brief"><b>'+esc(management.name)+'</b><span>'+esc(management.desc)+'</span><small>Uji awal: '+esc(env.name)+' · galur tetap anonim.</small></div>';
