...
export function homography(points,width=WIDTH,height=HEIGHT){
  ...
  for(let c=0;c<8;c++){
    let p=c;for(let j=c+1;j<8;j++)if(Math.abs(a[j][c])>Math.abs(a[p][c]))p=j;
    if(Math.abs(a[p][c])<1e-10)throw Error('Posisi titik tidak valid.');
    [a[c],a[p]]=[a[p],a[c]];
    const d=a[c][c];for(let j=c;j<=8;j++)a[c][j]/=d;
...
export function detectMarkers({data,width,height}){
  ...
  for(let i=0;i<mask.length;i++){
    ...
    mask[i]=r>85&&b>65&&r>g*1.45&&b>g*1.35&&Math.abs(r-b)<110?1:0;
  }
  for(let i=0;i<mask.length;i++){
    if(!mask[i]||seen[i])continue;
    const stack=[i];seen[i]=1;let n=0,sx=0,sy=0,minX=width,maxX=0,minY=height,maxY=0;
    while(stack.length){
      const k=stack.pop(),x=k%width,y=Math.floor(k/width);n++;sx+=x;sy+=y;
    ...
    if(n>=8&&n<mask.length*.025&&w/h>.35&&w/h<2.8&&n/(w*h)>.35)clusters.push({n,p:[sx/n,sy/n]});
  }
  ...
  clusters.sort((a,b)=>a.p[1]-b.p[1]);
  const top=clusters.slice(0,2).sort((a,b)=>a.p[0]-b.p[0]),bottom=clusters.slice(2).sort((a,b)=>a.p[0]-b.p[0]);
  const points=[top[0].p,top[1].p,bottom[1].p,bottom[0].p];
  homography(points);return points;
}
export function bilinearSample(image,u,v,target,k){
  const {width,height,data}=image;
  if(!Number.isFinite(u)||!Number.isFinite(v)||u<0||v<0||u>width-1||v>height-1)return false;
  const x0=Math.floor(u),y0=Math.floor(v),x1=Math.min(x0+1,width-1),y1=Math.min(y0+1,height-1),dx=u-x0,dy=v-y0;
  for(let c=0;c<3;c++)target[k+c]=
    data[4*(y0*width+x0)+c]*(1-dx)*(1-dy)+data[4*(y0*width+x1)+c]*dx*(1-dy)+
    data[4*(y1*width+x0)+c]*(1-dx)*dy+data[4*(y1*width+x1)+c]*dx*dy;
  target[k+3]=255;return true;
}
